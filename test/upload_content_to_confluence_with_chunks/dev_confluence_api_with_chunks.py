    ...
    try:
        ...
        if set_width and content_response.status_code == 200:
            for prop_key, task_label in [
                ...
                property_response = requests.post(props_url, json=post_payload, headers=headers)
                if property_response.status_code == 409:
                    # Property already exists — fetch its version and PUT instead
                    get_resp = requests.get(f"{props_url}/{prop_key}", headers=headers)
                    if get_resp.status_code == 200:
                        ...
                        property_response = requests.put(f"{props_url}/{prop_key}", json=put_payload, headers=headers)
